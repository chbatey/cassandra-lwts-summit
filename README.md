...
Replicas respond with a promise not to accept any proposals with a lower version.
...
Repliacs respond with a promise not to accept any lower proposal, given that:

* The replica has not promised to accept a higher proposal
...
```
INSERT INTO users (user_name, password, email ) values ( 'chbatey', 'different', 'adifferentchris@gmail.com' ) IF NOT EXISTS;
```

Finite resource:
```
CREATE table vouchers(name text PRIMARY KEY, quantity int, sold int);
```

```
UPDATE vouchers SET sold = 2 where name = 'free tv' IF sold != 300;
